...
public class GameEngineImpl implements GameEngine {
  ...
  public void reloadPlayerIntoGame(String gameToken, User reconnectingUser)
    ...
    enforceGameCheck(game != null && game.getId().equals(gameToken),
  ...
  @Override
  public void nominateLeader(final RoomName roomName, final User nominator, final User nominee)
      throws GameException {
    ...
    if (room.isPlayerInRoom(usurper) && room.isPlayerInRoom(leadershipNominee) && !room
      ...
      if (room.getUsurpAttempt() == null) {
        UsurpAttempt usurpAttempt = new UsurpAttempt(roomName, usurper, leadershipNominee,
            room.getPlayers(), getLeadershipVoteResolutionAction(this));
  ...
  public PostRoomVoteAction getLeadershipVoteResolutionAction(GameEngine gameEngine) {
    return new PostRoomVoteAction() {
      @Override
      public void success(RoomName roomName) {
        gameEngine.resolveLeadershipVote(roomName, LeadershipVote.YES);
      }

      @Override
      public void failure(RoomName roomName) {
        gameEngine.resolveLeadershipVote(roomName, LeadershipVote.NO);
      }
    };
  }
}